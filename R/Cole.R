...
#' Cole(x)
Cole <- function (X, Y = NULL, alpha = 0.95, Fisher = TRUE, covar = "iid", m_rep = 10000, c_seq = NA, n, details_inference = FALSE) {
  if (isFALSE(alpha)){
    if (!is.null(Y)){
      X <- table(X, Y)
    }
  ...
  # Inference with details
  if (details_inference == TRUE) {
    if (isTRUE(Fisher)){
      ...
      # Merge the p-values
      pZval_min_pretest <- pmin(1, 2 * apply(cbind(p_vals_pretest, apply(pZvals, 1, max, na.rm = T)), 1, min, na.rm=T))
      ...
      res <- dplyr::tribble(~C, ~CI_lower, ~CI_upper,
                            #--|--|--
                            C, CZ_CI[1], CZ_CI[2])
      details <- dplyr::tibble(c_seq = c_seq , pv_uneq = pZvals[,1], pv_eq = pZvals[,2], pv_pretest = p_vals_pretest)
